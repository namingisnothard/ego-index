...
const isRobotReference = (entry: LibraryEntry) => entry.referenceTags.includes('Robot reference');
...
export default function CompactDirectory() {
  ...
  return <main className={styles.compactPage}>
    ...
    <div className={styles.compactSearch}><input value={query} onChange={(event) => setQuery(event.target.value)} aria-label="Search the directory" placeholder="Search name, task, signal, venue…" /><select value={role} onChange={(event) => setRole(event.target.value)} aria-label="Filter by role"><option>All roles</option>{(network ? ['Person', 'Organization'] : ['Ego', 'Ego dataset', 'Robot reference', 'Pipeline', 'General model', 'Policy']).map((item) => <option key={item}>{item}</option>)}</select><select value={sort} onChange={(event) => setSort(event.target.value)} aria-label="Sort records"><option>Newest</option><option>Oldest</option><option>A–Z</option><option>Z–A</option></select>{(query || role !== 'All roles' || sort !== 'Newest') && <button type="button" onClick={() => { setQuery(''); setRole('All roles'); setSort('Newest'); }}>Clear</button>}</div>
